...
```js
// Responsive Scrubber component — inline styles only
function Scrubber(values, {format = value => value, initial = 0, direction = 1, delay = null, autoplay = true, loop = true, loopDelay = null, alternate = false} = {}) {
  ...
  const form = html`<form class="scrubber"
    style="
      font: 9px var(--sans-serif);
      display: flex;
    ...
    <button name=b type=button
      ...
      "></button>
    <label style="display: flex; flex-direction: column; gap: 0.2rem; box-sizing: border-box; min-width: 0;">
      <input name=i type=range min=0 max=${values.length - 1} value=${initial} step=1
        style="
          ...
          min-width: 0;
          box-sizing: border-box;
          -webkit-appearance: none; /* improve mobile rendering */
        ">
      <output name=o style="font-size: 10px; font-weight: 600; text-align: center; max-width: 100%; box-sizing: border-box; overflow: hidden; text-overflow: ellipsis;"></output>
    </label>
  </form>`;
...
const countrySelector = Inputs.select(countries, {
  value: "World",
  label: ""
});
```
 ...
 const isMobile = width < 640;
  ...
  if (currentCountry && selectedCountry !== "World") {
        ...
        const legend = svg.append("g")
          .attr("transform", `translate(${width - marginRight + 30}, ${marginTop + 10})`);
...
function topCountriesChart({ width } = {}) {
  ...
  return Plot.plot({
    ...
    // Give labels real breathing room
    ...
    y: {
      ...
      tickPadding: 8,          // pushes text away from axis
      tickSize: 0
    },
...
function bottomCountriesChart({ width } = {}) {
  ...
  const shortenCountry = d => {
    if (!isMobile) return d;
  ...
  return Plot.plot({
    ...
    // more space for long labels
    marginLeft: isMobile ? 120 : 180,
    marginRight: 40,
    marginBottom: 40,
...
function regionalComparisonChart({ width } = {}) {
  ...
  const shortenRegion = d => {
    if (!isMobile) return d;

    return d
      .replace("Sub-Saharan Africa", "Sub-Saharan Afr.")
      .replace("Latin America & Caribbean", "LatAm & Carib.")
      .replace("East Asia & Pacific", "East Asia & Pac.")
  ...
  return Plot.plot({
    ...
    marks: [
      Plot.barX(sortedRegions, {
        ...
        tip: true,
        title: d =>
          `${d.country}: ${d.value.toFixed(2)} births per 1,000 (${selectedYear})`
      }),
      ...
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-card-header">
           <div class="stat-card-label">Global Average</div>
          </div>
         <div class="stat-card-content">
            <div class="stat-card-value">${globalYearAvg.toFixed(1)}</div>
            <div class="stat-card-subvalue">births per 1,000 (${selectedYear})</div>
          </div>
        </div>
         <div class="stat-card">
         <div class="stat-card-header">
           <div class="stat-card-label">Decline Since 19960</div>
          </div>
          <div class="stat-card-content">
            <div class="stat-card-value">-${declineFromStart.toFixed(1)}</div>
            <div class="stat-card-subvalue">(${declinePercent}% reduction)</div>
          </div>
        </div>
        <div class="stat-card">
        <div class="stat-card-header">
           <div class="stat-card-label">${selectedCountry}</div>
          </div>
          <div class="stat-card-content">
            <div class="stat-card-value">${countryRate ? countryRate.toFixed(1) : "N/A"}</div>
            <div class="stat-card-subvalue">births per 1,000</div>
          </div>
        </div>
        <div class="stat-card">
        <div class="stat-card-header">
          ...
          <div class="stat-card-content">
            <div class="stat-card-value">${yearData.length}</div>
            <div class="stat-card-subvalue">in ${selectedYear}</div>
          </div>
        </div>
    </div>
          <div class="insights-card" style="flex: 1; padding: 16px;">
          <div class="insights-header">
            <h4 class="insights-title">Timeline</h4>
          </div>
...
<div class="insights-header">
    <h4 class="insights-title" style="margin-bottom: 0.5rem;">Compare Country</h4>
</div>
...
```
    ...
    <div class="insights-card">
        ...
        <ul class="insights-list">
          <li>Global Decline: World birth rates have fallen from ~${firstYearAvg.toFixed(0)} to ~${latestGlobalAvg.average.toFixed(0)} per 1,000 since ${minYear}</li>
          <li>Demographic Transition: Nearly all countries follow the pattern of declining fertility as development increases</li>
          <li>Regional Gaps: Sub-Saharan Africa still has rates &gt;35, while Europe is below 12</li>
          <li>Fastest Declines: East Asia saw the most dramatic reductions, led by China's one-child policy era</li>
        </ul>
      </div>
    </div>
    <div class="chart-card ">
        ...
        <div class="chart-title-row">
          <h3 class="chart-title"> 🌍 Global Birth Rate Trend (1960-2023) — Blue: Global Average, Orange: ${selectedCountry}</h3>
        </div>
        </div>
        <div class="chart-content" style="margin-top: 60px;">
       ${resize((width) => globalTrendChart({width}))}
</div>
     <div class="chart-header">
      <div class="chart-title-row">
          <h3 class="chart-title"> 🔴 Top 5 Highest Birth Rates (${selectedYear})</h3>
        </div>
    </div>
    <div class="chart-content" style="margin-top: 20px;">
     ...
     <div class="dashboard-bottom" style="margin-top:1rem;">
         <div class="chart-card">
            <div class="chart-header">
                <div class="chart-title-row">
